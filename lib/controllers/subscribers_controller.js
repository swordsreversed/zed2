...
module.exports = {
	...
	show: function(req, res) {
		...
		req.models.subscriber.get(req.params.id, function (err, sub) {
			if (!err) {
				res.json(sub);
			} else {
	...
	subsuggest: function(req, res) {
			var foo = { sublastname: orm.like(req.params.name + '%') };
			req.models.subscriber.find(foo).order('subfirstname').run(function (err, subs) {
				(!err) ? res.json(subs) : console.log(err);
		});
	 }
